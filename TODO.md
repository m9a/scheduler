...
| 20 | ⬜ | **Registration thundering-herd after outage (P1)** — when the coordinator returns, every worker re-registers and flushes all its unacked status updates at once; that burst could overwhelm the coordinator. Stagger/backoff registration (jitter the reconnect, or have the coordinator pace/batch the resync flush) so N workers don't hammer it simultaneously. Related to #18 (resubscribe backoff). |
